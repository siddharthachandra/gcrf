...
/**
 * @brief Computes the Energy (L2) loss @f$
 ...
 *      the computed Energy loss: @f$ E =
 *          \frac{1}{2n} \sum\limits_{n=1}^N \left| \left| \hat{y}_n - y_n
 *        \right| \right|_2^2 @f$
 *
 * This can be used for least-squares regression tasks.  An InnerProductLayer
 * input to a EnergyLossLayer exactly formulates a linear least squares
...
class EnergyLossLayer : public LossLayer<Dtype> {
 public:
  explicit EnergyLossLayer(const LayerParameter& param)
  ...
  /**
   * Unlike most loss layers, in the EnergyLossLayer we can backpropagate
 ...
 protected:
  ...
  /**
   * @brief Computes the Energy error gradient w.r.t. the inputs.
   *
   * Unlike other children of LossLayer, EnergyLossLayer \b can compute
